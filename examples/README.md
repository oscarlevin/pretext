...
### Numbering Stress Test (`numbering/`)

A synthetic document that exercises the numbering system as exhaustively as we could manage: every numbered family, in every numbering-relevant scope, built across the range of numbering levels and diffed.  Generated by Claude (Anthropic's AI) over a long run of `@serial` numbering testing; it is a machine-oriented test fixture, not an example of authoring practice (see the directory's own `README`).  (2026-06-01)


### Course Calendar (`calendar/`)

A short set of fictional course notes containing no real date at all, demonstrating the course calendar: symbolic date references (`week6.wed`, `class17`, `exam1+2d`) printed with `date` and `daterange`, and content gated on a date with `@release-id`.  The prose explains each construction as it is used, so the document reads as documentation, and four publication files build the same source at different points in the term.  The directory's own `README` carries independently computed tables of what each build should contain, which makes this a test as well as an example.  (2026-08-06)
