...
<xsl:param name="slides.everything" select="'no'"/>
<!-- Others to be added to control what content gets included -->


<!-- We will use chunk.level to determine how to split up the beamer files -->
...
<xsl:template match="assemblage" mode="beamer">
...
<xsl:template match="figure" mode="beamer">
...
<!-- Tables -->
<xsl:template match="table" mode="beamer">
    <xsl:text>\begin{frame}[plain]&#xa; &#xa;</xsl:text>
    <xsl:text>\vfill&#xa;</xsl:text>
...
<xsl:template match="tabular" name="tabular">
    ...
    <xsl:text>\adjustbox{max totalsize={0.9\textwidth}{0.9\textheight}}{\begin{tabular}{</xsl:text>
...
<xsl:template match="image[@source]" >
    ...
    <xsl:text>\adjustbox{max totalsize={0.9\textwidth}{0.9\textheight}}{\includegraphics[width=</xsl:text>
...
<!-- The following might need to be changed too, although we might not ever display these -->
<!-- PGML answer blank               -->
<!-- Example: [_____]{$ans}          -->
